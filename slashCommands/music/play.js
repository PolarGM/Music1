        ...
        try {
            ...
            if(!playlist) {
                if(!queue || queue.tracks.length == 0) { 
                    ...
                    return interaction.editReply({ ephemeral: false, content: `**Now Playing: __${song.title}__** - \`${song.durationFormatted}\``}).catch(() => null);
                }
                ...
                if(!queue || queue.tracks.length == 0) { 
                    ...
                    client.queues.set(interaction.guild.id, newQueue)
                    await client.playSong(interaction.member.voice.channel, song);
                    return interaction.editReply({ ephemeral: false, content: `**Now Playing: __${song.title}__** - \`${song.durationFormatted}\`\n> **Added \`${playlist.videos.length - 1} Songs\` from the Playlist:**\n> __**${playlist.title}**__`}).catch(() => null);
                }
